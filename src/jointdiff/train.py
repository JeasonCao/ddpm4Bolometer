"""
Training script for a single score network (signal or noise).

Train two networks independently:
    # Score_x: learns p(signal)
    python -u -m src.jointdiff.train \
        --data_dir /path/to/clean \
        --output_dir /path/to/score_x_runs \
        --epochs 100

    # Score_n: learns p(noise)
    python -u -m src.jointdiff.train \
        --data_dir /path/to/noise \
        --output_dir /path/to/score_n_runs \
        --epochs 100
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train unconditional score network for Joint Diffusion.")
    parser.add_argument('--data_dir', type=str, required=True,
                        help='Path to H5 data (clean pulses or noise windows)')
    ...
    config['method'] = 'jointdiff_score'
    with open(os.path.join(args.output_dir, 'config.json'), 'w') as f:
        json.dump(config, f, indent=2)

    # Dataset
    print("Loading dataset...")
    dataset = SingleSignalDataset(args.data_dir)
    n_total = len(dataset)
    ...
    if args.resume:
        ...
        if os.path.exists(history_path):
            ...
            if history.get('val_loss'):
                ...
                print(f"Initialized best_val_loss={best_val_loss:.6f}")
